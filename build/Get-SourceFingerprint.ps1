<#
  ...
  两条硬要求（pre.8 上线时各踩过一次）：
    1) 排除 bin/ 与 obj/ 里的编译生成文件（它们带本机绝对路径，换台机器就算不出同一个值）；
    2) 跨平台一致：路径一律用正斜杠，文件内容先把 CRLF 归一成 LF 再算哈希
       —— 这样 Windows 与 Linux（发布工作流跑在 ubuntu 上）算出的是同一个值。
...
try {
    foreach ($file in $files) {
        ...
        # 先按 UTF-8 读出文本、把 CRLF 归一成 LF，再算哈希：同一个文件在 Linux 检出（LF）
        # 与 Windows 检出（CRLF，仓库 .gitattributes 强制）下得到同一个值。
