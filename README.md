...
- 内容校验目标写在 `ProbeTargets` 里，格式 `http:主机/路径|期望文本`（`https:` 同样支持）
  或 `http:主机/路径|204`（只认状态码，专治「被劫持时返回 200 门户页」）：
  只有 HTTP 2xx **且**响应体包含期望文本才算通过，3xx 一律算失败（被跳到登录页正是未认证的特征）；
- 内容校验**第一次没过时会立刻补测一次**（此时 TCP 已经证明能连，多半只是单次超时）：
...
| `ProbeTargets` | 3 个内容校验目标 + 1 个 TCP 目标 | 判断「能不能上网」的目标；`http:主机/路径\|期望文本`、`http:主机/路径\|204` 为内容校验，`tcp:主机:端口`、`icmp:主机` 为兜底 |
...
| `ConfigVersion` | `4` | 配置结构版本（自动迁移：60 秒探测 → 20 秒；旧的默认探测列表 → 新的四条默认） |
