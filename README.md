...
- **网络正常时几乎不发请求**：默认每 20 秒做一次本地探测（先做**内容校验**：真的取回指定网页文本，再用 TCP / ICMP 兜底），
  平时**完全不碰 Portal**；在线时最多每 5 分钟只读核对一次 Portal 会话；
...
```
   ...
   本地探测第一步：内容校验（真的取回指定网页文本，或核对状态码 204）
             ...
             （0 个 Portal 请求）   ┌───────┴────────┐
              每 5 分钟只读核对   result=1         result=0 / 探测不通
              一次 Portal 会话        │                │
...
- 正常联网时**每小时约 180 次本地探测**（默认每 20 秒一次），Portal 只读核对约 12 次（默认每 5 分钟一次），
  登录请求为 **0**；
...
| `state.json` | 当前状态快照（在线与否、最近结果、最近会话核对、累计检查次数）；状态没有变化时最多每 60 秒才写一次盘 |
