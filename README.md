...
- **提交登录后确认得快**：先做一次 0 延迟的本地内容校验，再按**每档 2 秒、共 12 档**加密复检
  （累计 2/4/6/…/24 秒，每档只发一次**只读** `chkstatus`）。本机 211 次自然掉线实测：
  上一版 3~6 秒一档时「提交 → 确认」中位 **22 秒**，改成 2 秒一档后，真实信号一出现最多 2 秒内就被看见；
...
1. 到 [Releases](../../releases) 下载附件 `CampusNet-v2.1.0.exe`（2.x 的发布只有这一个文件）。
   附件名里的版本号和标签名一致（标签 `v2.1.0` → 附件 `CampusNet-v2.1.0.exe`），文件名不影响使用、
...
| 运行统计 | 最近探测 / 最近登录时间、最近结果、本小时登录次数、连续失败、下次探测、会话校验、探测方式、最近错误、已忽略提示、强制重登、今日登录、恢复耗时 |
...
- 登录提交后**先立刻做一次本地内容校验**（0 延迟、不发 Portal 请求），再按 **+2 / +4 / +6 … +24 秒（每档 2 秒）**复检：
