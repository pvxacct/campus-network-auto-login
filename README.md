...
- **网络正常时几乎不发请求**：默认每 20 秒只做一次本地探测（TCP 连接 + ICMP），**完全不碰 Portal**，不查状态、不登录；
- **断网时秒级重连**：探测失败 1 秒内连试多次，确认断开才去查 Portal 并自动登录；
- **不触发风控**：登录前二次确认、两次登录至少间隔 60 秒、每小时最多 12 次；遇到限流或「账号已在别处在线」只记下原因、本次不再重试，按上面的节奏继续尝试，不会反复顶号；
...
| 运行统计 | 最近探测 / 最近登录时间、最近结果、本小时登录次数、连续失败、下次探测、最近错误 |
...
- 正常联网时**每小时约 180 次本地探测**（默认每 20 秒一次），Portal 请求为 **0**；
...
| `state.json` | 当前状态快照（在线与否、最近结果、累计检查次数）；状态没有变化时最多每 60 秒才写一次盘 |
...
| `ConfigVersion` | `2` | 配置结构版本（首次运行到 2.0.0-pre.2 时会把旧的 60 秒探测自动升级为 20 秒） |
