...
| 运行统计 | 最近探测 / 最近登录时间、最近结果、本小时登录次数、连续失败、下次探测、会话校验、探测方式、最近错误 |
...
```
        ...
        └───────────────────┬──────────────────────┘
                            ▼
   本地探测第一步：内容校验（取回 msftconnecttest.txt 的指定文本）
                    ┌───────┴────────┐
               内容对得上          对不上（或被代答）
                    │                │
             ...
             （0 个 Portal 请求）   ┌───────┴────────┐
              ...
              一次 Portal 会话        │                │
                                  保持在线        等 3 秒二次确认 → POST /drcom/login
                                  （记下原因）             │
                                                  登录成功 → 复探确认
```
...
| `ProbeTargets` | 1 个内容校验目标 + 2 个 TCP 目标 | 判断「能不能上网」的目标；`http:主机/路径\|期望文本` 为内容校验，`tcp:主机:端口`、`icmp:主机` 为兜底 |
...
| `ConfigVersion` | `3` | 配置结构版本（自动迁移：60 秒探测 → 20 秒；旧的全 TCP 探测列表 → 带内容校验的新默认） |
