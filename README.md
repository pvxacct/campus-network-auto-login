...
- **网络正常时几乎不发请求**：默认每 10 秒做一次本地探测（先做**内容校验**：真的取回指定网页文本，再用 TCP / ICMP 兜底），
  平时**完全不碰 Portal**；在线时最多每 60 秒只读核对一次 Portal 会话；
- **掉线发现得快**：订阅系统网络变化事件（网线插拔 / WiFi 切换 / IP 变化）后 1 秒内复检一次；
  「内容校验没过」按异常节奏复探，并每 10 秒向 Portal 求证一次（只读），所以被踢下线后通常十几秒内就会开始重连；
- **断网时秒级重连**：所有探测目标**同时**跑（整轮耗时约等于单条目标的超时），
  HTTP 目标另有 3 秒的独立超时，连续几轮都不通才去查 Portal 并自动登录——完全断网时
  从发现到提交登录通常十几秒，而不是以前那种几十秒；
- **确认恢复不空等**：距上次登录不足 60 秒（风控硬性最小间隔）时，程序**每 3 秒只做本地探测**，
  不查 Portal、也不会登录；网络一恢复就立刻确认在线。老写法会让工作线程睡满剩余间隔——
  本机实测那段 33 秒空窗，正是「其实已经能上网，界面却还在重连」的元凶；
- **提交登录后确认得快**：先做一次 0 延迟的本地内容校验，再按 **前 6 档 1 秒、后 6 档 2 秒**
  （+1 … +6 / +8 … +18 秒）复检，每档的**只读** `chkstatus` 与本机内容校验**并行**跑。
  本机 211 次自然掉线实测「提交 → 确认」中位 **22 秒**，其中绝大部分是 Portal 自己的登录接口返回时间
  （20~30 秒，日志里会写明）；我们自己这部分只占几秒，且信号一出现最多 1~2 秒就被看见；
...
- **量得出**：运行统计里有「今日登录 N 次 / 提交 M 次」（跨零点自动重算）和
  「恢复耗时：上次 N 秒 / 本次中位 M 秒」（只留在内存里，退出即清空）；
...
1. 到 [Releases](../../releases) 下载附件 `CampusNet-v2.1.1.exe`（2.x 的发布只有这一个文件）。
   附件名里的版本号和标签名一致（标签 `v2.1.1` → 附件 `CampusNet-v2.1.1.exe`），文件名不影响使用、
...
| 账号 | 账号密码输入与保存（空框有灰色提示；已保存的账号只显示前三位，密码留空表示只改账号） |
| 运行统计 | 最近探测 / 最近登录时间、最近结果、本小时登录次数、下次探测、会话校验、今日登录、恢复耗时、最近错误 |
| 程序 | 开机自启开关、安装 / 卸载、打开数据目录、清理旧版残留、**高级设置**（单独一个窗口，改完即保存，没有「保存」按钮） |
| 运行日志 | 最近 400 行，WARN / ERROR 着色，增量追加；勾选「跟随最新」才会自动滚到底，可「只看警告和错误」，右上角可**清除显示**（只清空这个窗口的显示，不删本机日志文件） |
...
```
   ...
   本地探测第一步：内容校验（真的取回指定网页文本，或核对状态码 204）
             ...
             （0 个 Portal 请求）   ┌───────┴────────┐
               每 60 秒只读核对    result=1         result=0 / 探测不通
              一次 Portal 会话        │                │
...
- 正常联网时**每小时约 360 次本地探测**（默认每 10 秒一次），Portal 只读核对约 60 次（默认每 60 秒一次），
  登录请求为 **0**；掉线那一轮额外最多 14 次只读复检；
- 异常时每 3 秒探测一次，尽快把网络拉回来；**「TCP 通、内容校验没过」也按异常节奏复探**，
...
- Portal 回「账号已在别处在线 / 密码错误」这类业务提示时，程序**完全忽略**它：
  不当成失败、不写「最近错误」，只限频记一行日志，随后做几次轻量复检，网络真的通了就立刻算成功；
...
- 登录提交后**先立刻做一次本地内容校验**（0 延迟、不发 Portal 请求），再按
  **+1 / +2 / +3 / +4 / +5 / +6 / +8 / +10 / +12 / +14 / +16 / +18 秒**复检（前 6 档 1 秒、后 6 档 2 秒）：
  登录瞬间生效时不必先白等，Portal 稍后才认账的情况也能早几秒确认；每档的只读 `chkstatus` 与本机内容校验
  **并行**跑，整档耗时取两者较大值（不再是相加），日志会同时写出「第几档」和「真实经过秒数」；
- 被最小间隔（默认 60 秒）拦下时**不会干等**：等待期内每约 3 秒只做本地探测，
...
| `state.json` | 当前状态快照（在线与否、最近结果、最近会话核对及其来源、累计检查次数、今日登录成功 / 提交次数）；状态没有变化时最多每 60 秒才写一次盘 |
...
| `OnlineProbeSeconds` | `10` | 网络正常时的探测间隔 |
| `OfflineProbeSeconds` | `3` | 探测异常后的复检间隔（「疑似掉线复测」「最小间隔等待期轮询」也跟着它） |
...
| `SessionCheckSeconds` | `60` | 在线时只读核对 Portal 会话的间隔（`0` = 关闭）。这是「被踢下线」最主要的发现途径，调小恢复更快，调大请求更少 |
| `StuckReloginSeconds` | `60` | 本机连续不通但 Portal 说在线，超过这个秒数就自动注销重登（`0` = 关闭） |
| `ConfigVersion` | `8` | 配置结构版本（自动迁移，只改「等于旧默认值」的那一份：探测节奏 20 → 10 秒、异常探测 5 → 3 秒、会话核对 120 → 60 秒；旧的默认探测列表 → 新的四条默认；3 轮/1000 毫秒复检 → 2 轮/500 毫秒；登录前确认 3 秒 → 1 秒。自己填过的值一律保留） |
