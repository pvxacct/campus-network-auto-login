...
- **网络区**显示本机 IP 与网卡（含速率）、默认网关、网络名称与类别、DNS 服务器，以及到**网关**和 **Portal** 的实时延迟、最近若干次的平均／最小／最大延迟与丢包率（`PingEnabled=false` 可关闭）；
...
发一次 `chkstatus` 只读查询。它打包在单独的 zip 里，和自动登录包互不依赖，可以放在任意目录。
...
各版本的改动记录见 [CHANGELOG.md](CHANGELOG.md)，当前版本：**1.7.0**（2026-09-13）。
